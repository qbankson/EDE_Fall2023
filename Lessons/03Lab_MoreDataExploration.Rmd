...
> Answer: Matrices and data frames appear in the data section of the enviornment. Vectors and single value objects appearin values. 
...
```{r DataExercise-1a}
high_temp <- c(74, 80, 81, 81, 77, 72, 81, 80, 82, 81)
low_temp <- c(58, 61, 60, 59, 61, 64, 63, 63, 63, 63)
```
...
high_c <- (high_temp - 32) * 5/9
low_c <- (low_temp - 32) * 5/9
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
df_temp <- data.frame("High.TempF" = high_temp, "Low.TempF" = low_temp, low_temp, high_c, low_c)
```
