...
vis_miss(BR_complete)


temperature_m <- ggplot(BR_complete, aes(x=Date, y = Temperature_C)) +
  geom_line()

temperature_m


rainfall_m <- ggplot(BR_complete, aes(x=Date, y = Rainfall)) +
  geom_line()
...
temperature_m_sm <- ggplot(BR_complete, aes(x=Date, y = Temperature_C)) +
  geom_line()+
...
rainfall_m_sm <- ggplot(BR_complete, aes(x=Date, y = Rainfall)) +
  geom_line()+
  geom_smooth(method="lm")
...
first_m <- month(first(BR_complete$Date))
first_d <- year(first(BR_complete$Date))

t_seq_df <- ts(data = BR_complete$Temperature_C, start = c(first_m, first_d), frequency = 12)

r_seq_df <- ts(data = BR_complete$Rainfall, start = 1901, 12, end = 2016, frequency =12)
...
r_stl<- plot(stl(r_seq_df, "per"))

t_stl <- plot(stl(t_seq_df, "per"))
