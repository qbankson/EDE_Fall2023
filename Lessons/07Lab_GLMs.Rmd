...
ozone_dense <- ggplot(EPAozone, aes((x = Ozone), fill = as.factor(Year))) +
                      geom_density(alpha=0.5) +
                      scale_fill_manual(values = c("darksalmon", "blue")) +
                      labs(x="Ozone AQI Value", 
                           y="Density",
                           fill="")
ozone_dense

ozone_box <- ggplot(EPAozone, aes(x = as.factor(Year), y = Ozone)) + 
                      geom_boxplot() +
...
ozone_viol <- ggplot(EPAozone, aes(x = as.factor(Year), y = Ozone)) + 
...
NTL_LTER <-read.csv((file = "/Users/mac/Documents/EDE_Fall2023/Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv") , stringsAsFactors = TRUE)
...
PM_scatter <- ggplot(EPAair, aes(x = Ozone, y = PM2.5)) +
  geom_point()+
