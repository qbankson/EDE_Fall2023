...
#Months are numeric values. Theya are an integer class. 
...
ggplot(data=PeterPaul.chem.nutrients, aes(x=month))+
  geom_bar()
...
ggplot(data=PeterPaul.chem.nutrients, 
  aes(x=factor(PeterPaul.chem.nutrients$month, levels = 1:12, 
  labels = month.abb)))+
  geom_bar()
...
date_temp_plot <- PeterPaul.chem.nutrients %>% 
  ggplot(
    mapping = aes(
      x=sampledate,
      y=temperature_C,
